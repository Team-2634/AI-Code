...
public class limelight {
    ...
    double camYaw = getCamYaw() * (Math.PI/180);

    double distanceFromWall = camY * Math.cos(camYaw) - camX * Math.sin(camYaw);
    double distanceFromTag  = camX * Math.cos(camYaw) + camY * Math.sin(camYaw);

    return new double[]{distanceFromTag, distanceFromWall};
}
